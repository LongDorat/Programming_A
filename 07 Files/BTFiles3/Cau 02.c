...
int down(state_t state, state_t *new_state);
...
int main()
{
    ...
    state_t new_state;
    int moved = down(state, &new_state);
...
int down(state_t state, state_t *new_state)
{
    if (state.r == state.m - 1)
        return 0;

    new_state->m = state.m;
    new_state->n = state.n;
    new_state->r = state.r + 1;
    new_state->c = state.c;
    for (int i = 0; i < state.m; i++)
    {
        for (int j = 0; j < state.n; j++)
    ...
    new_state->a[state.r][state.c] = state.a[state.r + 1][state.c];
    new_state->a[state.r + 1][state.c] = 0;
    return 1;
}
